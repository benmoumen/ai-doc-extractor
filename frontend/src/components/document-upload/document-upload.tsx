  ...
  const validateFile = useCallback(
    (file: File): string | null => {
      if (!acceptedTypes.includes(file.type)) {
        return `File type ${
          file.type
        } is not supported. Please upload ${acceptedTypes.join(", ")}`;
      }
      ...
      if (file.size > maxSizeBytes) {
        return `File size ${(file.size / 1024 / 1024).toFixed(
          1
        )}MB exceeds maximum of ${maxFileSizeMB}MB`;
      }
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const result = await uploadDocumentWithProgress(request, (progress) => {
          setUploadState((prev) => ({
            ...prev,
        ...
        setTimeout(() => {
          ...
          const detectedType =
            result.analysis?.detected_document_type || "Unknown";
          const confidence = result.analysis?.document_type_confidence
            ? Math.round(result.analysis.document_type_confidence * 100)
            : 0;

          toast.success(
            `Document processed successfully! Detected: ${detectedType} (${confidence}% confidence)`
          );
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  <div>
                    ...
                    <span className="font-medium">
                      {uploadState.result.analysis?.detected_document_type ||
                        "Unknown"}
                    </span>
                  </div>
                  <div>
                    Confidence:{" "}
                    <span className="font-medium">
                      {uploadState.result.analysis?.document_type_confidence
                        ? Math.round(
                            uploadState.result.analysis
                              .document_type_confidence * 100
                          )
                        : 0}
                      %
                    </span>
                  ...
                  <div>
                    Fields Detected:{" "}
                    <span className="font-medium">
                      {uploadState.result.analysis?.total_fields_detected || 0}
                    </span>
                  </div>
                  <div>
                    Processing Time:{" "}
                    <span className="font-medium">
                      {uploadState.result.total_processing_time?.toFixed(1) ||
                        "0.0"}
                      s
                    </span>
                  </div>
