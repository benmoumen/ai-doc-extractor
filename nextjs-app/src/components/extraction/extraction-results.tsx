...
import {
  Copy, Download, Check, Edit2, Save, X, AlertCircle,
  ChevronDown, ChevronRight, Eye, EyeOff, Sparkles
} from 'lucide-react'
...
export function ExtractionResults({
  ...
  const [viewMode, setViewMode] = useState<'table' | 'json' | 'grouped'>('grouped')

  // Group fields by confidence level
  const groupedFields = {
    high: result.extractedFields.filter(f => (f.confidence || 0) >= 0.8),
    medium: result.extractedFields.filter(f => (f.confidence || 0) >= 0.5 && (f.confidence || 0) < 0.8),
    low: result.extractedFields.filter(f => (f.confidence || 0) < 0.5),
    unverified: result.extractedFields.filter(f => f.confidence === undefined)
  }
  ...
  const getSourceIcon = (source?: string) => {
    ...
    if (source === 'hybrid') return <Sparkles className="h-3 w-3 text-blue-500" />
    return null
  }
  ...
  const renderFieldValue = (field: ExtractedField) => {
    ...
    const value = field.type === 'object' || field.type === 'array'
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-2">
              ...
              {result.processingTime && ` in ${result.processingTime.toFixed(2)}s`}
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    {showConfidenceScores ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                ...
                {expandedGroups.has('high') && (
                  <div className="ml-6 space-y-2">
                    {groupedFields.high.map((field) => (
                      <div key={field.id} className="grid grid-cols-3 gap-4 p-3 border rounded-lg">
                        <div className="flex items-center gap-2">
                ...
                {expandedGroups.has('medium') && (
                  <div className="ml-6 space-y-2">
                    {groupedFields.medium.map((field) => (
                      <div key={field.id} className="grid grid-cols-3 gap-4 p-3 border rounded-lg bg-yellow-50 dark:bg-yellow-950/20">
                        <div className="flex items-center gap-2">
                          <Label className="text-sm font-medium">{field.displayName}</Label>
                          {getSourceIcon(field.source)}
                          {showConfidenceScores && getConfidenceBadge(field.confidence)}
                        </div>
                        <div className="col-span-2">
                          {renderFieldValue(field)}
                        </div>
                ...
                {expandedGroups.has('low') && (
                  <div className="ml-6 space-y-2">
                    {groupedFields.low.map((field) => (
                      <div key={field.id} className="grid grid-cols-3 gap-4 p-3 border rounded-lg bg-red-50 dark:bg-red-950/20">
                        <div className="flex items-center gap-2">
                          <Label className="text-sm font-medium">{field.displayName}</Label>
                          {getSourceIcon(field.source)}
                          {showConfidenceScores && getConfidenceBadge(field.confidence)}
                ...
                {result.extractedFields.map((field) => (
                  <TableRow key={field.id}>
                    ...
                    {showConfidenceScores && (
                      <TableCell>{getConfidenceBadge(field.confidence)}</TableCell>
                    )}
                    <TableCell>
          ...
          <>
            ...
            <div className="space-y-3">
              {result.warnings?.map((warning, idx) => (
                <div key={idx} className="flex items-start gap-2 p-3 bg-yellow-50 dark:bg-yellow-950/20 rounded-lg">
